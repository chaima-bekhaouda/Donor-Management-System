...
require_once __DIR__ . '/DTO/DonorDTO.php';
...
class DonorModel
{
     ...
     * @throws Exception
     */
    public function canDonateAgain(int $donorId): bool
    {
        ...
        $canDonateBlood = !$lastBloodDonationDate || ($now->diff($lastBloodDonationDate)->days >= 56);

        // Check if 28 days have passed since the last plasma donation
        $canDonatePlasma = !$lastPlasmaDonationDate || ($now->diff($lastPlasmaDonationDate)->days >= 28);

        return $canDonateBlood && $canDonatePlasma;
    }
